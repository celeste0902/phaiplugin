...
(function () {
  ...
  */
  ...
  function updateWbRightPanel() {
    ...
    var editor = wbEl.querySelector('#mufy-wb-editor');
    var snap = wbSnapshot[wbCurrentIndex];
    var origTokens = estimateTokens(snap.originalContent);
    var draftTokens = estimateTokens(editor.value);
    var delta = draftTokens - origTokens;

    wbEl.querySelector('#mufy-wb-orig-token').textContent = origTokens + ' token';
    wbEl.querySelector('#mufy-wb-draft-token').textContent = draftTokens + ' token';

    var deltaEl = wbEl.querySelector('#mufy-wb-token-delta');
    if (delta === 0) {
      deltaEl.textContent = '无变化';
      deltaEl.style.color = '#9a9aae';
    } else if (delta > 0) {
      deltaEl.textContent = '+' + delta + ' token';
      deltaEl.style.color = '#86efac';
    } else {
      deltaEl.textContent = delta + ' token';
